...
class Post extends React.PureComponent<IProps> {
  ...
  renderPost = (post: IPost, sources: TSource, user?: IUser, group?: IGroup, inner: boolean = false) => {
  ...
  onPictureClick = (e: React.MouseEvent<HTMLImageElement>) => {
    e.preventDefault();
    console.log('#POST > Clicked on picture', e.target, new Date(Date.now()).toLocaleTimeString());
  }
